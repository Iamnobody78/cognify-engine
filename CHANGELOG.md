...
### 整改 (基于代码实证批判, 逐项核验后修复)
- **测试一致性**: tests 三处 `== 7` 断言 → 动态 (`>= 7` + 核心 7 个必须存在), **10/10 全绿** (此前 10 插件时代必失败)
- **证书语义**: cert() 插件检查 `== 7` → `>= 7` + 版本 2.0.0→2.2.3 — **cert 实测从 NOT_CERTIFIED 恢复 CERTIFIED** (10 插件动态)
- **版本单一化**: manifest 2.0.0→2.2.3 / SELF_DESCRIPTION 2.1.0→2.2.3 / README 25 维→30 维 (消除 22/25/30 并存)
- **PyPI 打包**: packages 含 core/ (运行时依赖); _cli.py 降级提示诚实化 (wheel 不含 cli 时明确指引源码运行, 不再虚假指向); 完整打包列为 v2.3.0
- **pyproject BOM 修复**: tomllib 解析失败根因 (Set-Content 引入 BOM) → 去 BOM 重写
- **README 宣称收敛**: 30/30 active 与动态真相源一致

### 诚实承认 (批判成立项)
- 元能力判定仍为存在性代理 (P1: active=文件存在+冒烟, 列入整改); META-CALL 零注入仍 CERTIFIED (闭环=步骤执行报告, 产出消费追踪列为 P1); 硬编码路径 TRI/WS/PY 环境变量化列为 P0 后续

### 工程
- pytest 10/10 | cert CERTIFIED | README/SELF_DESCRIPTION/manifest/证书 版本同源 2.2.3
