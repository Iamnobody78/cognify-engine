...
class DaggerAgent(DQNAgent):
    ...
    def _anneal_beta(self):
        ...
        # T2: dagger_lambda co-anneals from start → end (1.0 → 0.2 typical)
        if self.dagger_lambda_end != self.dagger_lambda:
            self.dagger_lambda = max(
                self.dagger_lambda_end,
                self.dagger_lambda_start
                - self.total_steps
                * (self.dagger_lambda_start - self.dagger_lambda_end)
                / max(1, self.cfg.epsilon_decay),
            )
...
def main():
    ...
    ap.add_argument("--defensive-weight", type=int, default=2,
                    help="T1: defensive slots in curriculum pool (default 2, e.g. 4 = 4x sampling)")
    ap.add_argument("--lambda-decay", type=float, default=0.0,
                    help="T2: anneal dagger_lambda from --dagger-lambda to this value over training")
    ...
    print(f"╔{'═' * 64}╗")
    print(f"║  Sprint 41: defensive tuning + dagger_lambda decay (S40 base)  ║")
    print(f"╚{'═' * 64}╝")
    print(f"  init: {args.init_weights}")
    print(f"  beta: {args.beta_start} → {args.beta_end} (linear over {cfg.epsilon_decay} steps)")
    print(f"  dagger_lambda={args.dagger_lambda}"
          + (f" → {args.lambda_decay} (linear decay)" if args.lambda_decay > 0 else " (fixed)")
          + f"  skill_lambda={args.skill_lambda}")
    print(f"  defensive_weight={args.defensive_weight}  epsilon: {cfg.epsilon_start} → {cfg.epsilon_end} | episodes: {args.episodes}")

    agent = DaggerAgent(
        cfg, args.init_weights,
        skill_lambda=args.skill_lambda,
        dagger_lambda=args.dagger_lambda,
        dagger_lambda_end=args.lambda_decay if args.lambda_decay > 0 else None,
        beta_start=args.beta_start,
        beta_end=args.beta_end,
    )

    # curriculum: gate×2 + speed ladder×1, with defensive oversampling (T1)
    base = ["random", "aggressive", "defensive", "circler", "counter"] * 2
    if args.defensive_weight > 2:
        base += ["defensive"] * (args.defensive_weight - 2)
    curriculum_pool = base + ["moderate", "passive", "stationary"]
