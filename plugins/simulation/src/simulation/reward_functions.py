...
class V10Reward:
    ...
    def compute_edge_reward(
        ...
        # S38 T2 FIX (FP-RL-006): removed the unconditional probe-based backstop
        # `if edge_min < edge_critical/20.0 → -150 terminal`. It fired when a
        # sensor PROBE 7.5cm ahead of the robot center crossed the rim while the
        # robot CENTER was still on the dohyo (body 75% on) — i.e. a pursuer
        # chasing an opponent to the rim "died" mid-push and the episode was
        # judged a double-loss (-150) instead of the opponent falling off first
        # (+200 win). That bug made the defensive profile structurally
        # unwinnable (chase baseline 0/10, bait/flank 0/10, BC 0/8): every
        # edge-push died at the rim. The authoritative center-based flag from
        # the env is the ONLY fall signal now (all callers pass it).
